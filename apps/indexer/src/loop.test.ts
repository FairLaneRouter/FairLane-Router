...
import { nextSampleSlot, runSlotLoop } from './loop.ts'
 ...
 */
function fakeRpc(head: number | number[], failures: Map<number, Error> = new Map()): FakeRpc {
...
describe('runSlotLoop', () => {
  ...
  it('reports the failure with the slot it happened on', async () => {
    ...
    const rpc = fakeRpc(100, new Map([[100, new Error('502 від провайдера')]]))
